...
describe('API Testing', () => {
  ...
  // Pengujian untuk menghapus item
  it('should delete an item', (done) => {
    const itemId = 1; // Pastikan ID item sesuai
    request(app)
      .delete(`/api/items/${itemId}`) // Gunakan template literal
      .end((err, res) => {
        expect(res.status).to.equal(200); // Sesuaikan dengan status API Anda
        // Memeriksa apakah item benar-benar dihapus
        request(app)
          .get(`/api/items/${itemId}`) // Gunakan template literal
          .end((err, res) => {
  ...
  // Pengujian untuk memperbarui item
  it('should update an item', (done) => {
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        request(app)
          .put(`/api/items/${itemId}`) // Gunakan template literal
          .send(updatedItem)
          .end((err, res) => {
